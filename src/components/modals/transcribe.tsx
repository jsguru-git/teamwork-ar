...
class TranscribeModal extends React.Component<Props, State> {
      ...
      <CustomModal close={this.close}>
        <Container style={styles.container}>
          ...
          <View style={{ flex: 1, flexDirection: 'row' }}>
            ...
            <Button onPress={this.send} color='#7C91EC' style={styles.recBtn}>
              <Text>
                Send
            </Text>
            </Button>
          </View>
          <Button onPress={this.translate} color='#7C91EC' style={styles.recBtn}>
            <Text>
              Translate
            </Text>
          </Button>
  ...
  async stopRecording() {
    if (!this.state.isRecording) {
      console.warn('Not current recording.');
      return;
    }
    ...
    try {
      AudioRecorder.stopRecording().then((filePath: string) => {
  ...
  translate = () => {
    Api.instance.utils.translate(this.state.transcription, this.state.language)
      .then((res) => {
  ...
  send = () => {
    this.close();
    TwilioChat.instance.sendMessage(this.state.transcription);
    TwilioChat.instance.sendMessage(this.state.translation);
  }
